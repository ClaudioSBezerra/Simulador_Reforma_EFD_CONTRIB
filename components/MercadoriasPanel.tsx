  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white">Mercadorias (Supabase Real-Time)</h2>
          <p className="text-[#92adc9] text-sm">Cálculos projetados via Views Analíticas SQL.</p>
        </div>
        <select 
          value={selectedYear}
      ...
      <div className="flex border-b border-[#233648]">
        <button onClick={() => setActiveTab('0')} className={`px-6 py-3 text-sm font-bold transition-colors ${activeTab === '0' ? 'text-primary border-b-2 border-primary' : 'text-[#92adc9] hover:text-white'}`}>Entradas (Aquisição)</button>
        <button onClick={() => setActiveTab('1')} className={`px-6 py-3 text-sm font-bold transition-colors ${activeTab === '1' ? 'text-primary border-b-2 border-primary' : 'text-[#92adc9] hover:text-white'}`}>Saídas (Faturamento)</button>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left whitespace-nowrap">
              <thead className="bg-[#192633] text-[#92adc9] text-[10px] uppercase font-bold tracking-widest border-b border-[#233648]">
                <tr>
                  <th className="px-4 py-4">Filial / CNPJ</th>
                  <th className="px-4 py-4">Data Doc.</th>
                  <th className="px-4 py-4 text-right">Base ICMS</th>
              ...
              <tbody className="divide-y divide-[#233648]">
                {records.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-[#567089]">Nenhum registro encontrado para este critério.</td></tr>
                ) : (
                  records.map((rec) => (
                    <tr key={rec.id} className="hover:bg-[#1c2a38]">
                      ...
                      <td className="px-4 py-4 text-sm text-right font-mono text-white">{formatCurrency(rec.vl_bc_icms || 0)}</td>
                      <td className="px-4 py-4 text-sm text-right font-mono font-bold text-yellow-500">{formatCurrency(rec.vl_icms_projetado || 0)}</td>
                      <td className="px-4 py-4 text-sm text-right font-mono font-bold text-indigo-400">{formatCurrency(rec.vl_ibs_projetado || 0)}</td>
                      <td className="px-4 py-4 text-sm text-right font-mono font-bold text-emerald-400">{formatCurrency(rec.vl_cbs_projetado || 0)}</td>
                    </tr>
                  ))
                )}
              </tbody>
